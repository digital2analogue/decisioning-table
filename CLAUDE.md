...
**Visual baselines are generated ON the CI runner, never locally** — font rasterization differs across machines, so locally-generated baselines fail in CI. After an intentional visual change, run the "Update visual baselines" workflow from the Actions tab — **after your final push to the branch**: the workflow commits back to the branch it ran on, so pushing mid-run races its push (it now rebases + retries, but the run it raced on 2026-07-02 failed; dispatch it last and let it finish).

**Screenshot tolerance is ABSOLUTE (`maxDiffPixels: 200`), not a ratio — changed 2026-09-09, reasoning inline in `playwright.config.ts`.** The old `maxDiffPixelRatio: 0.02` allowed 25,920 px on a 1440×900 capture — a 160×160 block, bigger than most controls on the page. Measured on the runner at the first run under the new setting, all three baselines had drifted and were passing inside the old allowance: `onboarding` by **21,644 px** (83% of it), `table-demo` by **479 px**, `table-validation` by **342 px**. The two table numbers held identical across all three retries, so they are deterministic drift, not run-to-run noise. Those baselines date from 2026-07-02 and predate the whole token migration (#61) and the dnd-kit rewrite (#35) — they were never re-recorded because `--update-snapshots` only rewrites a baseline whose comparison *failed*, and the tolerance kept absorbing them. When a baseline now fails, **look at the diff before regenerating**: a failure is information, and blanket-regenerating blesses whatever is currently on screen.

**Corollary, learned the hard way here: a local screenshot comparison in this repo is not evidence.** The "generate baselines on the runner, never locally" rule also governs *reading* diffs. The numbers above were first measured in a dev container and came out almost exactly inverted — `onboarding` read 0 px locally against 21,644 on the runner, and the two table captures read ~23,000 px each locally against 479 and 342. A local run cannot tell you which baselines are stale; only a runner run can.

`main` has branch protection requiring the `checks` and `visual` status checks; Dependabot files weekly grouped bumps and the `dependabot-automerge` workflow merges them once CI passes (major npm bumps stay manual — and note `@dependabot` commands posted via API integrations are ignored; use the GitHub UI).
